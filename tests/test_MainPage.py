...
#запуск кода
#python -m pytest -v --driver Chrome --driver-path C:\DevSF_Python\Module_28\test_MainPage.py


"""Логотип Ozon (переход в раздел зона лучших цен) виден на странице"""
...
"""Переносит в раздел Big Promo при клике на лого Ozon"""
...
"""Кнопка TOPFashion работает и переносит в соответствующий раздел"""
...
"""Кнопка Билеты и Отели работает, перносит в соответствующий раздел"""
...
"""Кнопка Ozon счет работает, перносит в соответствующий раздел"""
...
"""Кнопка раздела Пункты выдачи видна на странице"""
def test_button_geo_on_page(driver):
    main_page = MainPage(driver)
    button = main_page.is_visible(MainPage.GEO)
    assert button == True


"""Кнопка Пункты выдачи имеет верное название"""
def test_button_geo_has_true_name(driver):
    main_page = MainPage(driver)
    button_name = main_page.get_text_of_element(MainPage.GEO)
    assert button_name == TestData.LIMIT


"""Кнопка Пункты выдачи работает и переносит в соответствующий раздел"""
def test_geo_clickable(driver):
    main_page = MainPage(driver)
    main_page.find_click(MainPage.GEO)
    page_header = main_page.get_text_of_element(MainPage.HEAD_OF_PAGE_GEO)
    assert page_header == TestData.GEO
